...
class Messages(commands.Cog): 
  ...
  async def webhook(self, channel) -> discord.Webhook:
      for webhook in await channel.webhooks():
        if webhook.user == self.bot.user:
          return webhook
      try: await channel.create_webhook(name='haunted')
      except: pass
  ...
  async def afk_listener(self, message: discord.Message):
     ...
     if message.mentions: 
      if len(message.mentions) == 1: 
        ...
        if check:
         em = discord.Embed(color=self.bot.color, description=f"💤 **{mem}** is AFK since <t:{check['time']}:R> - {check['reason']}")
     ...
     if che:
      embed = discord.Embed(color=self.bot.color, description=f"<a:wave:1020721034934104074> Welcome back **{message.author}**! You were AFK since <t:{che['time']}:R>")
  ...
  async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        import json
        key = f"edited_messages:{functions.hash(before.channel.id)}"
        await self.bot.redis.lpush(key, json.dumps(data))
        await self.bot.redis.expire(key, 3600)  # 1 hour expiry
  ...
  async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        ...
        import json
        key = f"removed_reactions:{functions.hash(payload.channel_id)}"
        await self.bot.redis.lpush(key, json.dumps(data))
        await self.bot.redis.expire(key, 3600)  # 1 hour expiry
  ...
  async def on_message_delete(self, message: discord.Message):
        ...
        import json
        key = f"deleted_messages:{functions.hash(message.channel.id)}"
        await self.bot.redis.lpush(key, json.dumps(data))
        await self.bot.redis.expire(key, 3600)  # 1 hour expiry
