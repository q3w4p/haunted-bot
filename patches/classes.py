...
class OwnerConfig:
    async def send_dm(ctx: commands.Context, member: discord.Member, action: str, reason: str): 
        embed = discord.Embed(color=ctx.bot.color, description=f"You have been **{action}** in every server haunted is in.\n{f'**Reason:** {reason}' if reason != 'No reason provided' else ''}")
...
class ValidWebhookCode(commands.Converter):
  async def convert(self, ctx: HauntedContext, argument: str):
...
class Player(pomice.Player):
  def __init__(self, *args, **kwargs):
   ...
   self.ctx: HauntedContext = None
   self.queue = pomice.Queue()
   self.loop: bool = False
   self.current_track: pomice.Track = None
   self.awaiting = False

  def set_context(self, ctx: HauntedContext):
    self.context = ctx
