...
def has_admin():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        if not admin: return await ctx.warning("You do not have **anti-nuke admin**")
        return True
...
def is_enabled():
    async def predicate(ctx: commands.Context) -> bool:
        ...
        if not module:
            return await ctx.warning("AntiNuke is not **enabled** in this server. Use `antinuke enable` to **enable** it.")
        return True
...
class antinuke(commands.Cog):
    ...
    @has_admin()
    @is_enabled()
    @antinuke.command(description='Add an Anti-Nuke Whitelisted Member', help="[User]", usage="@qnok", brief="Anti-Nuke Admin", aliases=['wl'])
    @Permissions.has_permission(administrator=True)
    ...
    @has_admin()
    @is_enabled()
    @antinuke.command(description='Add an Anti-Nuke Admin', help="[User]", usage="@qnok", brief="Anti-Nuke Admin")
    @Permissions.has_permission(administrator=True)
    ...
    async def whitelisted(self, ctx: commands.Context):
        ...
        if not whitelisted: return await ctx.warning("No users are **whitelisted** in this server.")
    ...
    async def admins(self, ctx: commands.Context):
        ...
        if not admins: return await ctx.warning("No users are **Anti-Nuke Admins** in this server.")
    ...
    async def enable(self, ctx: commands.Context):
        ...
        if enabled: return await ctx.warning("AntiNuke is already **enabled** in this server.")
        ...
        if not modules:
            for name in self.modules:
                ...
                await ctx.bot.db.execute("INSERT INTO antinuke_modules (guild_id, module, enabled, threshold, punishment) VALUES ($1, $2, $3, $4, $5)", ctx.guild.id, name, True, 3, 'ban')
    ...
    async def action(self, ctx: commands.Context, module: str, action: str):
        ...
        if module.capitalize() not in self.modules: return await ctx.warning(f"The module `{module}` is not a valid **Anti-Nuke module**.")
        if action.lower() not in ['ban', 'warn', 'kick' ,'strip']: return await ctx.warning(f"The action `{action}` is not a valid action. Use `ban`, `warn`, `kick` or 'strip'.")
