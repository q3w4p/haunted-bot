...
class information(commands.Cog):
    ...
    async def serverinfo(self, ctx: commands.Context):
        ...
        embed.description = f"Created {created_ago}"
        
        # Server name and description with line separator
        server_desc = guild.description if guild.description else "No Description"
        embed.add_field(
            name=f"**{guild.name}**",
            value=f"{server_desc}\n\n─────────────────────────",
            inline=False
        ...
        embed.add_field(
            name="",
            value=f"**{guild.member_count} members** • Level {guild.premium_tier}\n**{len(guild.text_channels)} text channels** • {len(guild.voice_channels)} voice channels\n**{len(guild.roles)} roles** • {len(guild.emojis)} emojis\n**{guild.premium_subscription_count} boosts** • {len(guild.categories)} categories",
            inline=False
        )
        
        # Vanity URL and Join Server buttons
        view = discord.ui.View()
        
        # Icon, Splash, Banner buttons first row
        if guild.icon:
        ...
        # Footer with owner and ID
        owner_name = f"@{guild.owner.name}" if guild.owner else "Unknown"
        embed.set_footer(text=f"Owner: {owner_name} • ID: {guild.id}")
        
        await ctx.reply(embed=embed, view=view if view.children else None)
    ...
    async def github(self, ctx, *, user: str):
        ...
        embed.set_thumbnail(url=avatar_url)
        embed.set_footer(text='Github', icon_url='https://cdn.haunted.dev/github.png')
        await ctx.reply(embed=embed)
    ...
    async def invite(self, ctx):
        embed = discord.Embed(color=self.bot.color)
        embed.description = f"Invite **Haunted** to your server."
        
        # Create view with invite button
        view = discord.ui.View()
        view.add_item(discord.ui.Button(
            emoji="🔗",
            url="https://discord.com/oauth2/authorize?client_id=1460976781531287553&scope=bot+applications.commands&permissions=8",
            style=discord.ButtonStyle.link
        ))
        
        await ctx.reply(embed=embed, view=view)
    
    @commands.command(aliases=["pos"], description='check member join position', usage="@qnok")
    ...
    async def botinfo(self, ctx: commands.Context):
        ...
        # Build embed matching the reference image style
        embed = discord.Embed(color=self.bot.color)
        embed.set_author(name=self.bot.user.name, icon_url=self.bot.user.display_avatar.url)
        
        # Description with developers
        embed.description = f"Developed & maintained by the [Haunted Team](https://discord.com/invite/hauntedbot)."
        
        # Statistics section
        stats_text = f"**Users:** {len(self.bot.users):,}\n**Commands:** {len(self.bot.commands)}\n**Servers:** {len(self.bot.guilds):,}"
        embed.add_field(name="Statistics", value=stats_text, inline=True)
        
        # Bot info section
        import discord as discord_lib
        bot_text = f"**Latency:** {latency}ms\n**Discord.py:** {discord_lib.__version__}\n**Created:** <t:{int(self.bot.user.created_at.timestamp())}:R>"
        embed.add_field(name="Bot", value=bot_text, inline=True)
        
        # Set thumbnail
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        
        # Footer with version
        embed.set_footer(text=f"Haunted v1.0.0", icon_url=self.bot.user.display_avatar.url)
        
        # Create view with buttons
        view = discord.ui.View()
        view.add_item(discord.ui.Button(label="Support", url="https://discord.com/invite/hauntedbot", emoji="💬"))
        view.add_item(discord.ui.Button(label="Website", url="https://haunted.gg", emoji="🔗"))
    ...
    async def avatarhistory(self, ctx: commands.Context, *, user: discord.Member | discord.User = None):
        ...
        async with ctx.typing():
            ...
            if not image or sys.getsizeof(image.fp) > ctx.guild.filesize_limit:
                await ctx.neutral(
                    (
                        f"Click [**here**](https://haunted.cc/avatars/{user.id}) to view"
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        ...
                        f" including GIFs click [**HERE**](https://haunted.cc/avatars/{user.id})"
                    ),
                )
                embed.set_author(
                    name=f"{user} ({user.id})",
