...
class utility(commands.Cog):
    ...
    async def userinfo(self, ctx: Context, *, member: Union[Member, User]=None):
        """Show user information with Created/Joined columns matching the reference design"""
        await ctx.typing()
        if member is None: 
            member = ctx.author
        ...
        # Build embed with clean design matching reference image
        e = Embed(color=self.bot.color)
        
        # Title: just the username
        e.title = str(user.name)
        ...
        # Created time
        created_delta = now - user.created_at.replace(tzinfo=timezone.utc)
        created_years = created_delta.days // 365
        created_months = (created_delta.days % 365) // 30
        created_days = created_delta.days % 30
        
        if created_years > 0:
            created_relative = f"{created_years} year{'s' if created_years != 1 else ''} ago"
        elif created_months > 0:
            created_relative = f"{created_months} month{'s' if created_months != 1 else ''} ago"
        else:
            created_relative = f"{created_days} day{'s' if created_days != 1 else ''} ago"
        
        created_date = user.created_at.strftime("%B %d, %Y")
        
        # Joined time (if member of guild)
        if isinstance(member, Member) and member.joined_at:
            joined_delta = now - member.joined_at.replace(tzinfo=timezone.utc)
            joined_years = joined_delta.days // 365
            joined_months = (joined_delta.days % 365) // 30
            joined_days = joined_delta.days % 30
            
            if joined_years > 0:
                joined_relative = f"{joined_years} year{'s' if joined_years != 1 else ''} ago"
            elif joined_months > 0:
                joined_relative = f"{joined_months} month{'s' if joined_months != 1 else ''} ago"
            else:
                joined_relative = f"{joined_days} day{'s' if joined_days != 1 else ''} ago"
            
            joined_date = member.joined_at.strftime("%B %d, %Y")
        else:
            joined_relative = "N/A"
            joined_date = "N/A"
        ...
        if isinstance(member, Member):
            roles = member.roles[1:][::-1]  # Exclude @everyone, reverse order
            if len(roles) > 0:
                # Show role mentions, limit to 5 with "..." if more
                if len(roles) <= 5:
                    role_text = " ".join([r.mention for r in roles])
                else:
                    role_text = " ".join([r.mention for r in roles[:5]]) + f" +{len(roles)-5}"
                e.add_field(name="Roles", value=role_text, inline=False)
            else:
                e.add_field(name="Roles", value="@+", inline=False)
    ...
    @commands.command(description="give someone permission to post pictures in a channel", usage="@qnok <channel>", brief="manage roles")
    ...
    @commands.command(description="see when a user was last seen", usage="@qnok")
    ...
    async def editsnipe(self, ctx: commands.Context, index: int = 1):
        ...
        raw_messages = await self.bot.redis.lrange(f"edited_messages:{functions.hash(ctx.channel.id)}", 0, -1)
        if not raw_messages:
        ...
        message = sorted_messages[index - 1]

        embed = discord.Embed(
            color = self.bot.color,
            description=(message.get("content") or ("__Message contained an embed__" if message.get("embeds") else "")),
            timestamp=datetime.datetime.fromtimestamp(message.get("timestamp", 0)),
        )
        
        author_data = message.get("author", {})
        embed.set_author(
            name=author_data.get("name", "Unknown"),
            icon_url=author_data.get("avatar") or None,
        )
        ...
        embed.set_footer(
            text=f"{index:,} of {len(messages)} message(s)",
    ...
    async def reactionsnipe(self, ctx: commands.Context, *, message: discord.Message = None):
        ...
        raw_reactions = await self.bot.redis.lrange(f"removed_reactions:{functions.hash(ctx.channel.id)}", 0, -1)
        ...
        if not message:
            ...
            message = ctx.channel.get_partial_message(reaction.get("message", 0))
        else:
            reaction = next(
                (r for r in reactions if r.get("message") == message.id),
                None,
            )
            if not reaction:
        ...
        try:
            await ctx.neutral(
                ...
                f" {discord.utils.format_dt(datetime.datetime.fromtimestamp(reaction.get('timestamp', 0)), style='R')}"
            )
    ...
    async def snipe(self, ctx: commands.Context, index: int = 1):
        ...
        raw_messages = await self.bot.redis.lrange(f"deleted_messages:{functions.hash(ctx.channel.id)}", 0, -1)
        if not raw_messages:
    ...
    async def weather(self, ctx: Context, *, location: str): 
     ...
     try:
         ...
         if "error" in data:
             return await ctx.warning(f"Could not find weather for **{location}**")
     except Exception as e:
         return await ctx.warning(f"Failed to fetch weather data: {e}")
