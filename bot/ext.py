...
class Client(object): 
    ...
    async def success(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message: 
        if isinstance(ctx, commands.Context): 
            return await ctx.reply(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.yes} {ctx.author.mention}: {message}"))
        else: 
            return await ctx.response.send_message(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.yes} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
  
    async def error(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message: 
        if isinstance(ctx, commands.Context): 
            return await ctx.reply(embed=discord.Embed(color=self.bot.error_color, description=f"{self.bot.no} {ctx.author.mention}: {message}"))
        else: 
            return await ctx.response.send_message(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.no} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
  
    async def warning(self, ctx: Union[commands.Context, discord.Interaction], message: str, ephemeral: bool=True) -> discord.Message: 
        if isinstance(ctx, commands.Context): 
            return await ctx.reply(embed=discord.Embed(color=self.bot.error_color, description=f"{self.bot.warning} {ctx.author.mention}: {message}"))
        else: 
            return await ctx.response.send_message(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.warning} {ctx.user.mention}: {message}"), ephemeral=ephemeral)
...
class GoToModal(discord.ui.Modal, title="change the page"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        if int(self.page.value) > len(self.embeds): 
            return await interaction.client.ext.warning(interaction, f"You can only select a page **between** 1 and {len(self.embeds)}", ephemeral=True) 
...
class PaginatorView(discord.ui.View): 
    ...
    async def goto(self, interaction: discord.Interaction, button: discord.ui.Button): 
        if interaction.user.id != self.ctx.author.id: 
            return await interaction.client.ext.warning(interaction, "You are not the author of this embed")     
        modal = GoToModal()
    ...
    @discord.ui.button(emoji="<:left:1451659678164582694>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:right:1451659703124885697>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:deny:1451659356448755763>", style=discord.ButtonStyle.secondary)
